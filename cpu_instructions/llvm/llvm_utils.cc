...
DEFINE_string(cpu_instructions_llvm_arch, "",
              "The architecture, for which the code is compiled.");
DEFINE_string(cpu_instructions_llvm_triple, "",
...
StatusOr<const llvm::Target*> GetLLVMTarget() {
  const std::string triple_name =
      FLAGS_cpu_instructions_llvm_triple.empty()
          ? llvm::sys::getDefaultTargetTriple()
          : std::string(FLAGS_cpu_instructions_llvm_triple);
  llvm::Triple triple(GetNormalizedLLVMTripleName());

  std::string error_message;
  const llvm::Target* const target = llvm::TargetRegistry::lookupTarget(
      FLAGS_cpu_instructions_llvm_arch, triple, error_message);
  if (target == nullptr) {
