...
TEST(TransformFactoryTest, GetTransformsFromCommandLineFlags) {
  FLAGS_cpu_instructions_transforms = "";
  EXPECT_EQ(GetTransformsFromCommandLineFlags().size(), 0);
  FLAGS_cpu_instructions_transforms = "TestTransform1";
  EXPECT_EQ(GetTransformsFromCommandLineFlags().size(), 1);
  FLAGS_cpu_instructions_transforms = "TestTransform2";
  EXPECT_EQ(GetTransformsFromCommandLineFlags().size(), 1);
  FLAGS_cpu_instructions_transforms = "TestTransform1,TestTransform2";
...
TEST(TransformFactoryDeathTest, GetTransformsFromCommandLineFlagsDoesNotExist) {
  FLAGS_cpu_instructions_transforms = "DoesNotExist";
